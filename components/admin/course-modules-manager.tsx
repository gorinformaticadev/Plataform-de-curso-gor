...
export function CourseModulesManager({
  ...
  // Função de drag and drop com persistência automática
  const handleDragEnd = async (result: any) => {
    ...
    try {
      ...
      const [draggedModule] = newModules.splice(sourceIndex, 1);
      newModules.splice(destinationIndex, 0, draggedModule);
      ...
      // Chamar endpoint de reordenação atômica
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/modules/reorder`,
        {
          modules: reorderedModules.map(({ id }, index) => ({ 
            id, 
            order: index 
          }))
        },
      ...
      if (response.data.success) {
        // Mover toast para useEffect para evitar setState durante render
        setTimeout(() => {
        ...
        // Simular atualização dos módulos na página pai
        // Como não temos onModulesChange, vamos forçar um reload suave
        setTimeout(() => {
  ...
  // Renderizar módulo
  const renderModule = (module: Module, index: number, dragHandleProps?: any) => {
                      ...
                      {modules.map((module, index) => (
                        <Draggable key={module.id} draggableId={module.id} index={index}>
                          {(provided) => (
                            ...
                            >
                              {renderModule(module, index, provided.dragHandleProps)}
                            </div>
                          )}
                        </Draggable>
                      ))}
